...
2. **状态与时间驱动（逻辑层）**
   ...
   * `WeatherTimeline` 使用世界 Seed 与带独立持续时间的阶段配置确定性地产生 `clear`、`rain`、`storm`；更新频率不会改变天气序列。自动天气中晴天单段持续 3–5 分钟，普通雨持续 2–3 分钟，暴雨持续约 1.5 分钟且占完整周期不足 10%。
   * 时间线以完整天气阶段周期为范围保存规范化的有限时间值，并使用固定精度时间单位累加，避免不同 Tick 步长在阶段边界产生漂移。
...
* **降雨亮度层级**：普通雨只允许在当前昼夜亮度基础上轻度压暗并略微增加雾浓度；显著变暗与高密度降水只属于低频 `storm` 阶段，禁止普通雨复用暴雨级环境参数。
