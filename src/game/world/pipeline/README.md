...
graph TD
    ...
    F -->|6. 在合适地表上生长树木并生成叶片| G[列天空光终算阶段]
    G -->|7. 按实际体素自上而下重算打包天空光| H[生成完成并返回 chunk]
```

1. **高度与群系图计算**：计算 16x16 范围内的插值高度、调整高度、局部水面，并初始化 `terrainMap` 与 `biomeMap` 共享上下文。
2. **地形基底填充**：读取地形图，在对应的三维空间中填充基岩、石头、泥土、沙子和海水，并写入**种子级**天空光近似值。
...
7. **列天空光终算（`ColumnSkyLightStage`）**：**必须放在管道最后**。洞穴/树木会改写体素却不更新光照；本阶段按与 `World.recalculateColumnSkyLight` 一致的衰减规则，对每个 (x,z) 列自上而下重算打包天空光（保留 block light 低 4 位）。上方 sub-chunk 不在内存时，用高度图近似入射光。禁止在装饰阶段各自写死光照魔法数后跳过终算。
