...
缺失、版本不符或 revision 过期的摘要一律视为“未知”。异步流送把未知区块作为当前直接可见前沿：区块自身会进入 `directVisible`，其后一层仍由安全外圈预加载，但在有效摘要返回前禁止继续跨区块传播。Worker 返回透明摘要后，`World` 标记可见性缓存失效，下一次解析才沿对应 portal 扩展前沿；不透明摘要则自然停止。这避免首次解析沿连续未知区块铺满整个视锥，同时保证透明洞口后方区块与破坏遮挡方块后的下一层已经就绪。无存活 Worker 或显式同步加载时允许保守穿越未知摘要，维持同步回退的完整加载能力。

摘要变化本身不得推进 `streamingEpoch`。重新解析只增加前沿区块时复用当前 epoch，使仍有效的生成和网格任务继续执行；只有区块离开 `active` 或显式清空缓存时才推进 epoch 并取消旧任务。因此透明摘要驱动的渐进扩展不会反复重启 Worker 队列，无拓扑影响的摘要更新也不会取消有效任务。
