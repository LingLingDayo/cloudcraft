...
`RuntimeKernel` 根据 `RuntimeSystem.dependencies` 进行拓扑排序，按依赖顺序初始化和更新，并按相反顺序释放。显式状态机禁止重复初始化和运行期注册。初始化失败时已初始化系统会立即回收；若回收也失败，`AggregateError` 会以初始化异常为 `cause`，同时保留初始化与全部清理错误。释放阶段会尝试处理所有系统后再汇总异常。
